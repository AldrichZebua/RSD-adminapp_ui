...
export type SafeApiResponse<T> = {} & (SafeApiSuccessResponse<T>) | (SafeApiErrorResponse);
...
export const safeApiRequest = async <T>(
  ...
  if (administrator === undefined) {
    return { success: false, redirect: true, data: { message: 'invalid session' } };
  }
  ...
  if (response.status === 401 || response.status === 402) {
    return { success: false, redirect: true, data: { message: 'unauthorized access' } };
  }
